...
if (!$checador_result || !$conductor_result) {
    ...
    echo "<script>window.location.href='horario_combi.html'</script>";
    exit();
}

// Insertar los datos en la tabla registro_personal_tb
$insert_query = "INSERT IGNORE INTO hora_sal_tb (id_trans, ruta, hr_sal, fecha, id_checador, id_conductor) VALUES (?, ?, ?, ?, ?, ?)";
$insert_stmt = $pdo->prepare($insert_query);
try {
    $insert_stmt->execute([$id_transporte, $ruta, $hr_sal, $fecha, $id_checador, $id_conductor]);
